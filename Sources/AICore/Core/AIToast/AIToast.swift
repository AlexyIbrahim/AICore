...
extension AIToast {
    ...
    public static func dismissDashBoard(dashBoardView: UIView) {
        UIView.animate(withDuration: 1.5, delay: 0, options: .curveEaseOut) {
            AIToast.dashView.frame = CGRect(x: dashSpacingCopy, y: (dashBoardView.frame.height/3)*2 + 1000, width: dashBoardView.frame.width-20, height: dashBoardView.frame.height/3)
        }
    }
}
