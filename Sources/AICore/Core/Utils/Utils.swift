...
public class Utils {
	...
	public final class func downloadImage(imagePath: String) -> Promise<ImageResult> {
		guard let imageURL = URL(string: imagePath) else {
			...
			promise.reject(NSError(domain: "Invalid URL", code: 0, userInfo: nil))
			return promise
		}
		return Utils.downloadImage(imageURL: imageURL)
	}
	
	public final class func downloadImage(imageURL: URL) -> Promise<ImageResult> {
		let promise = Promise<ImageResult>()
		ImageDownloader.default.downloadImage(with: imageURL) { result in
			switch result {
			case let .success(value):
				promise.fulfill(ImageResult(image: value.image, data: value.originalData, result: value))
