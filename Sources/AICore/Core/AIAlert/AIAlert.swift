    ...
    public final class func displayNativeMessage(message: String? = nil, withTitle title: String? = nil, style: UIAlertController.Style = .alert, buttons: [AlertButton], textFields: [AlertTextField]? = nil) {
        DispatchQueue.main {
            ...
            for alert_button in buttons {
                alertViewController.addAction(UIAlertAction(title: alert_button.title, style: alert_button.style) { _ in
...
public extension AIAlert.AlertButton {
    static let okay = AIAlert.AlertButton(title: "Okay", style: .default, callback: nil)
    static let cancel = AIAlert.AlertButton(title: "Cancel", style: .cancel, callback: nil)
    static let discard = AIAlert.AlertButton(title: "Discard", style: .cancel, callback: nil)
    static func yes(callback: ((_ textfields: [UITextField]?) -> Void)? = nil) -> AIAlert.AlertButton {
        return AIAlert.AlertButton(title: "Yes", style: .default, callback: callback)
    }

    static func no(callback: ((_ textfields: [UITextField]?) -> Void)? = nil) -> AIAlert.AlertButton {
        return AIAlert.AlertButton(title: "No", style: .default, callback: callback)
    }
}
